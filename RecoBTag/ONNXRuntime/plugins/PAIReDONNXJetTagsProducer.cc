...
private:
  ...
  unsigned min_pf_cands = 12;
  unsigned max_svs = 10;
  unsigned min_svs = 1;
  std::vector<std::vector<int64_t>> input_shapes_ = {{(int64_t)1, (int64_t)n_features_pf_, -1},
                                                     {(int64_t)1, (int64_t)n_features_vector_, -1},
                                                     {(int64_t)1, (int64_t)n_mask_, -1},
                                                     {(int64_t)1, (int64_t)n_features_sv_, -1},
                                                     {(int64_t)1, (int64_t)n_features_vector_,-1},
                                                     {(int64_t)1, (int64_t)n_mask_,  -1}};
  ...
  std::vector<std::string> output_names = {"label_cc", "label_bb", "label_bx", "label_cx", "label_ll"};
...
void PAIReDONNXJetTagsProducer::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  desc.add<edm::FileInPath>("model_path", edm::FileInPath("RecoBTag/Combined/data/PAIReD/PAIReD_c500.onnx"));
...
void PAIReDONNXJetTagsProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (size_t i = 0; i < input_sizes.size(); ++i) {
    const auto &len = input_sizes_.emplace_back(input_sizes[i]);
  ...
  std::vector<float> bb_score, cc_score, other_score, ll_score, cx_score, bx_score;
  ...
  for (unsigned i_jet = 0; i_jet < jets->size() - 1 && jets->size() > 0; ++i_jet) {
    ...
    for (unsigned j_jet = i_jet + 1; j_jet < jets->size(); ++j_jet) {
      ...
      bb_score.emplace_back(outputs[1]);
      cc_score.emplace_back(outputs[0]);
      ll_score.emplace_back(outputs[4]);
      bx_score.emplace_back(outputs[2]);
      cx_score.emplace_back(outputs[3]);
  ...
  pjTable->addColumn<float>("cx_score", cx_score, "Model score for cx jet", 10);
  pjTable->addColumn<float>("bx_score", bx_score, "Model score for bx jet", 10);
...
void PAIReDONNXJetTagsProducer::make_inputs(edm::Event& iEvent,
  ...
  for (size_t entry_i = 0; pf_idx.size() < 128 && entry_i < pf_sorted_idx.size(); ++entry_i) {
    ...
    } else if (true && (isClusteredToJet(cand, jet1) || isClusteredToJet(cand, jet2) || (*cand).pt() > 500)) {
      pf_idx.emplace_back(entry);
      //std::cout<<"new entry "<<entry_i<<" pt "<<(*cand).pt()<<std::endl;
    } else {
      continue;
  ...
  for (size_t sv_i = 0; sv_i < svs.size() && sv_idx.size() < max_svs; ++sv_i) {
    ...
    if (dl_cut.significance() <= 3.0) {
      continue;
    } else if (inEllipse(jet1.eta(), jet1.phi(), jet2.eta(), jet2.phi(), sv.eta(), sv.phi())) {
      sv_idx.emplace_back(sv_i);
    } else {
      continue;
    }
  }
  // adjust input shapes to the correct size
  unsigned cand_input_size = std::clamp((unsigned)pf_idx.size(), min_pf_cands, max_pf_cands);
  unsigned sv_input_size = std::clamp((unsigned)sv_idx.size(), min_svs, max_svs);
  input_shapes_[kPfCandFeatures][2] = static_cast<int64_t>(cand_input_size);
  input_shapes_[kPfCandVectors][2] = static_cast<int64_t>(cand_input_size);
  input_shapes_[kPfCandMasks][2] = static_cast<int64_t>(cand_input_size);
  input_shapes_[kSVFeatures][2] = static_cast<int64_t>(sv_input_size);
  input_shapes_[kSVVectors][2] = static_cast<int64_t>(sv_input_size);
  input_shapes_[kSVMasks][2] = static_cast<int64_t>(sv_input_size);
  // initialize some pointers used to fill data
  float* ptr = nullptr;
  float* start = nullptr;
  // loop through stored candidates and save all the input information
  for (size_t i_cand = 0; i_cand < pf_idx.size(); ++i_cand) {
    // get pf cand
    ...
    auto packed_cand = dynamic_cast<const pat::PackedCandidate*>(cand);
    //std::cout<<"candidate "<<i_cand<<": entry "<<entry<<std::endl;
    // enter features information into input data
    ptr = &data_[kPfCandFeatures][i_cand];
    start = ptr;
    *ptr = catch_infs_and_bound((log(packed_cand->pt()) - 1.0) * 0.25, 0, -5, 5);
    //std::cout<<"log pt "<<(*ptr)<<std::endl;
    ptr += pf_idx.size();
    *ptr = catch_infs_and_bound((log(packed_cand->energy()) - 2.0) * 0.25, 0, -5, 5);
    //std::cout<<"log e "<<(*ptr)<<std::endl;
    ptr += pf_idx.size();
    *ptr = catch_infs_and_bound((log(packed_cand->pt() / (jet1_pt + jet2_pt)) + 4.7) * 0.25, 0, -5, 5);
    //std::cout<<"log pt rel "<<(*ptr)<<std::endl;
    ptr += pf_idx.size();
    *ptr = catch_infs_and_bound((log(packed_cand->energy() / (jet1_energy + jet2_energy)) + 4.7) * 0.25, 0, -5, 5);
    //std::cout<<"log e rel "<<(*ptr)<<std::endl;
    ptr += pf_idx.size();
    *ptr = catch_infs_and_bound(reco::deltaR(*packed_cand, jet1), 0, -5, 5);
    //std::cout<<"deltaR1 "<<(*ptr)<<std::endl;
    ptr += pf_idx.size();
    *ptr = catch_infs_and_bound(reco::deltaR(*packed_cand, jet2), 0, -5, 5);
    //std::cout<<"deltaR2 "<<(*ptr)<<std::endl;
    ptr += pf_idx.size();
    *ptr = catch_infs_and_bound(packed_cand->charge(), 0, -1e32, 1e32);
    //std::cout<<"charge "<<(*ptr)<<std::endl;
    ptr += pf_idx.size();
    if (packed_cand->charge() != 0 && packed_cand->hasTrackDetails()) {
      *ptr = catch_infs_and_bound(std::tanh(packed_cand->dxy()), 0, -1e32, 1e32);
      //std::cout<<"d0 "<<(*ptr)<<std::endl;
      ptr += pf_idx.size();
      *ptr = catch_infs_and_bound(packed_cand->dxyError(), 0, 0, 1);
      //std::cout<<"d0 error "<<(*ptr)<<std::endl;
      ptr += pf_idx.size();
      *ptr = catch_infs_and_bound(std::tanh(packed_cand->dz()), 0, -1e32, 1e32);
      //std::cout<<"dz "<<(*ptr)<<std::endl;
      ptr += pf_idx.size();
      *ptr = catch_infs_and_bound(packed_cand->dzError(), 0, 0, 1);
      //std::cout<<"dz error "<<(*ptr)<<std::endl;
    } else {
      *ptr = std::tanh(-1);
      //std::cout<<"d0 "<<(*ptr)<<std::endl;
      ptr += pf_idx.size();
    ...
    ptr += pf_idx.size();
    if (jet1.eta() < 0) {
      *ptr = catch_infs_and_bound(jet1.eta() - (packed_cand->eta()), 0, -1e32, 1e32);
    } else {
      *ptr = catch_infs_and_bound((packed_cand->eta() - jet1.eta()), 0, -1e32, 1e32);
    }
    //std::cout<<"etarel1 "<<(*ptr)<<std::endl;
    ptr += pf_idx.size();
    *ptr = catch_infs_and_bound((reco::deltaPhi(packed_cand->phi(), jet1.phi())), 0, -1e32, 1e32);
    //std::cout<<"phirel1 "<<(*ptr)<<std::endl;
    ptr += pf_idx.size();
    if (jet2.eta() < 0) {
      *ptr = catch_infs_and_bound(jet2.eta() - (packed_cand->eta()), 0, -1e32, 1e32);
    } else {
      *ptr = catch_infs_and_bound((packed_cand->eta() - jet2.eta()), 0, -1e32, 1e32);
    }
    //std::cout<<"etarel2 "<<(*ptr)<<std::endl;
    ptr += pf_idx.size();
    *ptr = catch_infs_and_bound((reco::deltaPhi(packed_cand->phi(), jet2.phi())), 0, -1e32, 1e32);
    //std::cout<<"phirel2 "<<(*ptr)<<std::endl;
    ptr += pf_idx.size();
    *ptr = catch_infs_and_bound(packed_cand->puppiWeight(), 0, -1e32, 1e32);
    //std::cout<<"puppi weight "<<(*ptr)<<std::endl;
    assert(start + (n_features_pf_ - 1) * pf_idx.size() == ptr);
    // enter vector information into input data
    ptr = &data_[kPfCandVectors][i_cand];
    start = ptr;
    *ptr = catch_infs_and_bound(packed_cand->px(), 0, -1e32, 1e32);
    //std::cout<<"px "<<(*ptr)<<std::endl;
    ptr += pf_idx.size();
    *ptr = catch_infs_and_bound(packed_cand->py(), 0, -1e32, 1e32);
    //std::cout<<"py "<<(*ptr)<<std::endl;
    ptr += pf_idx.size();
    *ptr = catch_infs_and_bound(packed_cand->pz(), 0, -1e32, 1e32);
    //std::cout<<"pz "<<(*ptr)<<std::endl;
    ptr += pf_idx.size();
    *ptr = catch_infs_and_bound(packed_cand->energy(), 0, -1e32, 1e32);
    //std::cout<<"energy "<<(*ptr)<<std::endl;
    assert(start + (n_features_vector_ - 1) * pf_idx.size() == ptr);
  ...
  for (size_t sv_i = 0; sv_i < sv_idx.size(); ++sv_i) {
    const auto& sv = svs.at(sv_idx[sv_i]);
    ...
    ptr = &data_[kSVFeatures][sv_i];
    start = ptr;
    *ptr = catch_infs_and_bound(sv_charge, 0, -1e32, 1e32);
    ptr += sv_idx.size();
    *ptr = catch_infs_and_bound(sv.vertexNormalizedChi2() * 0.25, 0, -5, 5);
    ptr += sv_idx.size();
    *ptr = catch_infs_and_bound(d3d_meas.value() * 0.05, 0, -5, 5);
    ptr += sv_idx.size();
    *ptr = catch_infs_and_bound((d3d_meas.value() / d3d_meas.error()) * 0.01, 0, -5, 5);
    ptr += sv_idx.size();
    *ptr = catch_infs_and_bound(dxy_meas.value() * 0.05, 0, -5, 5);
    ptr += sv_idx.size();
    *ptr = catch_infs_and_bound((dxy_meas.value() / dxy_meas.error()) * 0.01, 0, -5, 5);
    ptr += sv_idx.size();
    *ptr = catch_infs_and_bound(sv.eta(), 0, -1e32, 1e32);
    ptr += sv_idx.size();
    *ptr = catch_infs_and_bound(sv.mass() * 0.1, 0, -5, 5);
    ptr += sv_idx.size();
    *ptr = catch_infs_and_bound(sv.vertexNdof() * 0.1, 0, -5, 5);
    ptr += sv_idx.size();
    *ptr = catch_infs_and_bound(sv.numberOfDaughters() * 0.1, 0, -1e32, 1e32);
    ptr += sv_idx.size();
    *ptr = catch_infs_and_bound(std::acos(-1 * vertexDdotP(sv, pv)) * 0.33, 0, -5, 5);
    ptr += sv_idx.size();
    *ptr = catch_infs_and_bound(sv.phi(), 0, -1e32, 1e32);
    ptr += sv_idx.size();
    *ptr = catch_infs_and_bound(sv.pt() * 0.01, 0, -5, 5);
    ptr += sv_idx.size();
    *ptr = catch_infs_and_bound(sv.position().x() * 0.2, 0, -5, 5);
    ptr += sv_idx.size();
    *ptr = catch_infs_and_bound(sv.position().y() * 0.2, 0, -5, 5);
    ptr += sv_idx.size();
    *ptr = catch_infs_and_bound(sv.position().z() * 0.2, 0, -5, 5);
    ptr += sv_idx.size();
    if (jet1.eta() < 0) {
      *ptr = catch_infs_and_bound(jet1.eta() - sv.eta(), 0, -1e32, 1e32);
    } else {
      *ptr = catch_infs_and_bound(sv.eta() - jet1.eta(), 0, -1e32, 1e32);
    }
    ptr += sv_idx.size();
    if (jet2.eta() < 0) {
      *ptr = catch_infs_and_bound(jet2.eta() - sv.eta(), 0, -1e32, 1e32);
    } else {
      *ptr = catch_infs_and_bound(sv.eta() - jet2.eta(), 0, -1e32, 1e32);
    }
    ptr += sv_idx.size();
    *ptr = catch_infs_and_bound((reco::deltaPhi(sv.phi(), jet1.phi())), 0, -1e32, 1e32);
    ptr += sv_idx.size();
    *ptr = catch_infs_and_bound((reco::deltaPhi(sv.phi(), jet2.phi())), 0, -1e32, 1e32);
    assert(start + (n_features_sv_ - 1) * sv_idx.size() == ptr);
    // save secondary vertex vectors to input data array
    ptr = &data_[kSVVectors][sv_i];
    start = ptr;
    *ptr = catch_infs_and_bound(sv.px(), 0, -1e32, 1e32);
    ptr += sv_idx.size();
    *ptr = catch_infs_and_bound(sv.py(), 0, -1e32, 1e32);
    ptr += sv_idx.size();
    *ptr = catch_infs_and_bound(sv.pz(), 0, -1e32, 1e32);
    ptr += sv_idx.size();
    *ptr = catch_infs_and_bound(sv.energy(), 0, -1e32, 1e32);
    assert(start + (n_features_vector_ - 1) * sv_idx.size() == ptr);
    // update mask and sv index
    ptr = &data_[kSVMasks][sv_i];
    *ptr = 1;
    ++sv_i;
  }
  // resize data vector
  data_[kPfCandFeatures].resize(n_features_pf_*cand_input_size);
  data_[kPfCandVectors].resize(n_features_vector_*cand_input_size);
  data_[kPfCandMasks].resize(n_mask_*cand_input_size);
  data_[kSVFeatures].resize(n_features_sv_*sv_input_size);
  data_[kSVVectors].resize(n_features_vector_*sv_input_size);
  data_[kSVMasks].resize(n_mask_*sv_input_size);
}
